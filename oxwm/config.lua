...
local blocks = {
  ...
  oxwm.bar.block.shell({
    format = "WIFI: {}",
    command = "nmcli device | awk '/^wlan0/{ print $4 }'",
    interval = 10,
    color = colors.red,
    underline = true,
...
oxwm.rule.add({ instance = "gl", floating = true })
...
-- Launch rofi
oxwm.key.bind({ "Mod1" }, "Space", oxwm.spawn({ "rofi -show drun" }))
...
-- Bar toggle
oxwm.key.bind({ modkey, "Shift" }, "B", oxwm.toggle_bar())
